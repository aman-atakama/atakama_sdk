...
def test_package_main(request, self_signed_cert, tmp_path):
    ...
    (final,) = [
        ...
        if file.suffix == ".apkg" and "runcmd" in str(file)
    ]
...
def test_not_valid_cert(self_signed_cert):
    ...
    p = Packager()
    with pytest.raises(atakama.packager.errors.ValidationError):
